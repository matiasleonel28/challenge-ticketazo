describe('Registro de Comprador', () => {
  ...
  const completarFormulario = (datos = {}) => {
    ...
    cy.get('[data-cy="input-fecha-nacimiento"]').within(() => {
      cy.get('[aria-label="día, "]').clear().type(datosFinal.dia);
      cy.get('[aria-label="mes, "]').clear().type(datosFinal.mes);
      cy.get('[aria-label="año, "]').clear().type(datosFinal.año);
    });
  ...
  // Buscar por data-slot sin importar el ID dinámico
  cy.get('[data-slot="error-message"]').should('have.length.at.least', 1)
    .and('be.visible')
